...
export default async function TransfersPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const ownClubTeamId = session?.user?.id
    ? await resolveUserClubTeamId(session.user.id)
    : null;
  const params: TransferSearchParams = {
    ...
    excludeTeamId: ownClubTeamId ?? undefined,
  ...
  return (
    ...
    <div>
      ...
      <div className="mt-5 space-y-3">{result.players.length ? result.players.map((player) => (
        <PlayerCardRow
          key={player.id}
          player={player}
          ownClubTeamId={ownClubTeamId}
        />
      )) : <div className="rounded-xl border border-dashed border-[var(--theme-border)] p-8 text-center text-[var(--theme-muted)]">No hay jugadores con estos filtros.</div>}</div>
